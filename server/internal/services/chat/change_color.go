...
// @Summary     Change chat color
// @Description Update the color of a chat
// @Tags        chat
// @Router      /api/v1/chat/{id}/color [PATCH]
// @Security    ApiKeyAuth
// @Param       id path uint true "Chat ID"
// @Param       body body dto.ChangeChatColorRequest true "New color"
// @Success     204
// @Failure     400 {object} dto.HttpError
// @Failure     500 {object} dto.HttpError
func (h *Handler) HandleChangeChatColor(c *fiber.Ctx) error {
	...
	req := new(dto.ChangeChatColorRequest)
	if err := c.BodyParser(req); err != nil {
		return apperror.BadRequest("invalid request body", err)
	}
	if err := h.validate.Struct(req); err != nil {
		return apperror.BadRequest("invalid color", err)
	}

	err = h.store.DB.WithContext(ctx).
		Model(&model.Chat{}).
		Where("id = ?", chatID).
		Update("color", req.Color).Error
	if err != nil {
		return apperror.Internal("failed to update color", err)
	}
